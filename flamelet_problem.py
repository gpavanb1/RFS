...
class FlameletProblem():
    ...
    def f(self, u, lmbda):
        ...
        """
        flamelet = Flamelet(FlameletSpec(library_slice=self.steady_lib, stoich_dissipation_rate=math.exp(lmbda)))
    ...
    def df_dlmbda(self, u, lmbda):
        ...
        for i in range(0, neq-1):
            species_list = np.hstack((self.air_y[i], u[i+1::neq], self.fuel_y[i]))
            current_species_ypp = 0.5*np.divide(self.DD(species_list)[1:], self.grid[1:])
    ...
    def flamelet_from_state(self, u):
        ...
        Used to plot 
        """
        flamelet = Flamelet(FlameletSpec(library_slice=self.steady_lib, stoich_dissipation_rate=math.exp(self.lmbda0)))
